...
/****** Object:  StoredProcedure [dbo].[Sp_refresh_currentsped]    Script Date: 3/1/2018 10:21:46 PM ******/
SET ANSI_NULLS ON
GO
...
--Exec [dbo].[Sp_refresh_currentsped]


Exec [dbo].[Sp_refresh_currentsped]

Alter procedure [dbo].[Sp_refresh_currentsped]
as
begin
...
select * from enrollments where USi='1512842621'
...
update a set a.EligibilityEventID = elig.id from #eacc a
inner join
(select * from #MaxIEP_EligEvents where eventtype = '1' and RANK = 1) elig
on a.studentcode = elig.studentcode and a.[district code] = elig.[district code]
...
select * from #univ_ini where Studentcode='1035635490'
...
if object_id('tempdb..#HServices_Total_Mins_exp_86') is not null
drop table #HServices_Total_Mins_exp_86
...
from #618_FinalReport f join (select distinct studentcode , leacode,[Special Ed Eligibility Status] from #univ_sped where isnull([Special Ed Eligibility Status],'')='YES'
)b on f.Studentcode = b.studentcode and f.LEACode = b.LEACode
...
update Sped_Current set leacode='1' where leacode='001'


------------------------ Update the Qlik table that uses SpEd data.
...
from [OSSEETLPROD01].[OSSE_Data_Exchange].[dbo].[RosterUncertifiedMaster] where isnull(USI,'')<>''
 ...
 if object_id('tempdb..#504_Plan') is not null
 drop table #504_plan
...
from Sped_Current s  left join enrollments e  on s.Studentcode = e.USI and cast(s.leaCode as int) = e.lea_code
...
select * from data_validation_SWD d  join [OSSEDAARPRD01].seds_data_exchange.dbo.sledspecialedstudents sl on d.studentcode= sl.usi and d.LEACode = sl.[district code]
where isnull(d.[current SWD Level],'' )<> isnull(sl.[Special_Education_level],'')



select * from #univ_sped where studentcode='1418959635'

select sl.* from data_validation_SWD d   right join [OSSEDAARPRD01].seds_data_exchange.dbo.sledspecialedstudents sl on d.studentcode= sl.usi and d.LEACode = sl.[district code]
where isnull(d.[Current SWD level],'')='' and isnull(sl.[Special_Education_level],'')<>'' 
and isnull(Sl.Rec_Srvs_Idea,'')=1


select * from [OSSEDAARPRD01].seds_data_exchange.dbo.sledspecialedstudents sl 

where isnull([Highest SWD Level],'')='' and isnull([current SWD Level],'')<>''


select * from districtcodebylea


select * from Sped_Current where studentcode='1035635490'
select * from data_validation_SWD where studentcode='1035635490'